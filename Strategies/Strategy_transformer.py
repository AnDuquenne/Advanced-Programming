...
class TimeSeriesTransformerForecaster(nn.Module):
    ...
    def __init__(self,
        ...
        self.transformer_encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=self.EMBEDDING_SIZE, nhead=self.NHEAD, dim_feedforward=2048,
                                       dropout=self.DROPOUT, batch_first=True),
            num_layers=self.NUM_LAYERS
        )

        self.transformer_decoder = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(d_model=self.EMBEDDING_SIZE, nhead=self.NHEAD, dim_feedforward=2048,
                                       dropout=self.DROPOUT, batch_first=True),
            num_layers=self.NUM_LAYERS
        )
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        assert x.size(2) <= self.max_len, "The input size is not correct"
        ...
        if self.DEBUG:
            ...
            print_size("pe_resized", self.pe[:, :x.size(1), :])
            print_size("x + pe_resized", x + self.pe[:, :x.size(1), :])
...
if __name__ == '__main__':
    ...
    print(f"Loading data from {config['Strategy']['Transformers']['data_path_X_encoder']}")
    print(f"Loading data from {config['Strategy']['Transformers']['data_path_X_decoder']}")
    print(f"Loading data from {config['Strategy']['Transformers']['data_path_y']}")
    ...
    # trainer.train()
    trainer.evaluate(X_encoder_test[:, :, :], X_decoder_test[:, :, :], y_test[:, :, :])
