"""
FC model for stock forecasting trainer
"""
...
class Trainer:
    def __init__(self, train_loader, test_loader, model, optimizer, criterion, scheduler, device, n_epoch,
        ...
        if self.wandb_:
            # Initialize wandb
            wandb.init(project="Adv_prog_FC", entity="anduquenne")
                ...
                if idx == len(self.train_loader) - 1:
                    with torch.no_grad():
                        ...
                        for idx_test, (input_test, target_test) in enumerate(self.test_loader):
                            ...
                            preds_test = self.model(input_test).to(self.device)
    ...
    def evaluate(self, X_batch, y_batch):
        ...
        with torch.no_grad():
            ...
            print("signal_encoder", X_batch.size())
